ElevateApp.controller('ExerciseCtrl', ['$scope', function($scope) {
	...
	$scope.exercises = [
		{'name': 'add 3 + 2','result': '5', 'type': 'Addition', 'choices': [3,4,5,2]},
		{'name': 'add 5 + 9','result': '14', 'type': 'Addition', 'choices': [13,14,15,21]},
		{'name': 'add 1 + 9','result': '10', 'type': 'Addition', 'choices': [30,40,15,10]},
...
//-------------- EVALUATE AND RETURN TRUE. We do this because we are using the active class in the html which needs to be either true or false ----------------//
	...
	$scope.verifyFieldOfKnowledge = function (field) {
		return $scope.fieldOfKnowledge !== null && $scope.fieldOfKnowledge.name === field.name;
	};
...
//First of all this file is made available via the <script> on the html. We use the variable name to attach this controller to MyFirstModule.
//The name of the controller is HomeCtrl.
//Scope as Data-Model
//$scope is the glue between application controller and the view. Now we can use the properties .execises and .areasOfKnowledge with {{}} to pass that data to the html.